...
exports.deleteEnquiry = exports.updateEnquiry = exports.getEnquiryById = exports.getAllEnquiries = exports.createEnquiry = void 0;
const Enquiry_1 = require("../models/Enquiry");
const createEnquiry = async (req, res) => {
    try {
        const { customerName, email, phone, message } = req.body;
        const newEnquiry = await Enquiry_1.Enquiry.create({
            customerName,
...
const getEnquiryById = async (req, res) => {
    try {
        const enquiry = await Enquiry_1.Enquiry.findById(req.params.id).populate("createdBy", "name email").populate("assignedTo", "name email");
        if (!enquiry)
            return res.status(404).json({ ok: false, msg: "Enquiry not found" });
...
const updateEnquiry = async (req, res) => {
    try {
        ...
        await enquiry.save();
        const populatedEnquiry = await Enquiry_1.Enquiry.findById(enquiry._id)
            .populate("createdBy", "name email")
            .populate("assignedTo", "name email");
        res.json({
            ok: true,
...
const deleteEnquiry = async (req, res) => {
    try {
        if (req.user?.role !== "admin")
            return res.status(403).json({ ok: false, msg: "Only admins can delete" });
        const enquiry = await Enquiry_1.Enquiry.findByIdAndUpdate(req.params.id, { deletedAt: new Date() }, { new: true });
        if (!enquiry)
            return res.status(404).json({ ok: false, msg: "Enquiry not found" });
